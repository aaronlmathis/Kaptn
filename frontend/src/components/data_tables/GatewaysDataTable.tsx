...
// Gateway type badge helper
function getGatewayServerTypeBadge(ports: Array<{ name?: string; protocol: string }>) {
...
// Column definitions for gateways table
const createColumns = (
	onViewDetails: (gateway: z.infer<typeof gatewaySchema>) => void
): ColumnDef<z.infer<typeof gatewaySchema>>[] => [
		{
			id: "drag",
			header: () => null,
...
function DraggableRow({ row }: { row: Row<z.infer<typeof gatewaySchema>> }) {
	const {
		transform,
		transition,
		setNodeRef,
...
export function GatewaysDataTable() {
	const { data: gateways, loading, error, refetch, isConnected } = useGatewaysWithWebSocket(true)
	...
	const [sortableIds, setSortableIds] = React.useState<UniqueIdentifier[]>(
		gateways.map((gateway: z.infer<typeof gatewaySchema>) => gateway.id)
	)

	React.useEffect(() => {
		setSortableIds(gateways.map((gateway: z.infer<typeof gatewaySchema>) => gateway.id))
	}, [gateways])
	...
	return (
		<div className="px-4 lg:px-6">
									...
									<SortableContext items={sortableIds} strategy={verticalListSortingStrategy}>
										{table.getRowModel().rows?.length ? (
											...
											<TableRow>
												...
												>
													No gateways found in {selectedNamespace === 'all' ? 'any namespace' : `namespace "${selectedNamespace}"`}.
												</TableCell>
											</TableRow>
										)}
									</SortableContext>
			...
			{/* Controlled detail drawer for full gateway details */}
