...
console.log('🔌 Testing single WebSocket with both cluster and node subscriptions...');
		...
		try {
			...
			if (message.type === 'ack' && message.groupId === 'capacity-headroom' && !clusterAckReceived) {
				...
				setTimeout(() => {
					...
					console.log('📤 Sending node subscription (20 series) on SAME connection...');
					console.log('📊 Total series now: 8 cluster + 20 node = 28 series');
