...
func (h *SessionInjectionHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
    ...
    // Do not redirect here. We inject session and let the frontend handle
    // routing to /login when unauthenticated. This avoids false positives from
    // cookie edge cases and keeps behavior consistent with our static SPA.
...
func (h *SessionInjectionHandler) injectSessionData(content string, sessionData *auth.MinimalUser, nonce string) string {
    // Create session data without CSRF token (using cookie-based CSRF instead)
