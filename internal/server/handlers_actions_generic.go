...
func (s *Server) HandleExecuteActions(w http.ResponseWriter, r *http.Request) {
    ...
    if req.RequestID == "" { req.RequestID = reqID }
    ...
    if conc <= 0 { conc = s.config.Actions.DefaultConcurrency }
    if conc > s.config.Actions.MaxConcurrency { conc = s.config.Actions.MaxConcurrency }
    ...
    for i:=0; i<conc; i++ { go worker() }
    go func(){
        for _, t := range targets { jobs <- job{ref: t} }
        close(jobs)
    }()

    out := make([]dto.ItemResult, 0, len(targets))
    for range targets { out = append(out, <-results) }
...
func (s *Server) HandleValidateGenericActions(w http.ResponseWriter, r *http.Request) {
    ...
    for i:=0; i<conc; i++ { go worker() }
    go func(){ for _, t := range targets { jobs <- job{ref: t} }; close(jobs) }()
...
func validateParams(req dto.GenericActionRequest) []dto.ParamError {
    ...
    switch a {
    case "patch":
        if _, ok := req.Params["patch"]; !ok { errs = append(errs, dto.ParamError{Field: "params.patch", Message: "required"}) }
        if pt, ok := req.Params["patchType"].(string); ok {
            switch strings.ToLower(pt) { case "merge", "strategic", "json": default: errs = append(errs, dto.ParamError{Field: "params.patchType", Message: "must be merge|strategic|json"}) }
        }
    case "scale":
        if _, ok := req.Params["replicas"]; !ok { errs = append(errs, dto.ParamError{Field: "params.replicas", Message: "required"}) }
    case "delete":
...
func (s *Server) resolveTargets(ctx context.Context, clients *k8s.ImpersonatedClients, refs []dto.ObjectRef, sel *dto.Selector) ([]dto.ObjectRef, error) {
    ...
    key := func(r dto.ObjectRef) string { return r.APIVersion+"|"+strings.ToLower(r.Kind)+"|"+r.Namespace+"|"+r.Name }
    for _, r := range refs { dedup[key(r)] = r }
    if sel != nil {
        // Map GVK -> GVR
    ...
    for _, r := range dedup { out = append(out, r) }
    return out, nil
}
...
func (s *Server) executeOneAction(ctx context.Context, clients *k8s.ImpersonatedClients, user string, groups []string, req dto.GenericActionRequest, ref dto.ObjectRef, validateOnly bool) dto.ItemResult {
    ...
    if strings.ToLower(req.Action) == "annotate" {
        ...
        if ann == nil { return dto.ItemResult{Ref: ref, Status: "error", HTTPStatus: http.StatusBadRequest, Message: "params.annotations must be object"} }
        req.Action = "patch"
    ...
    if strings.ToLower(req.Action) == "label" {
        ...
        if lab == nil { return dto.ItemResult{Ref: ref, Status: "error", HTTPStatus: http.StatusBadRequest, Message: "params.labels must be object"} }
        req.Action = "patch"
        req.Params["patchType"] = "merge"
    ...
    if strings.ToLower(req.Action) == "patch" {
        ...
        m, err := mapper.RESTMapping(gvk.GroupKind(), gvk.Version)
        if err != nil { return dto.ItemResult{Ref: ref, Status: "error", HTTPStatus: http.StatusBadRequest, Message: err.Error()} }
        ...
        case "strategic": pt = types.StrategicMergePatchType
        case "json": pt = types.JSONPatchType
        default: pt = types.MergePatchType
        }
        // Marshal patch body
    ...
    if strings.ToLower(req.Action) == "delete" {
        ...
        if v, ok := req.Params["propagationPolicy"].(string); ok {
            ...
            case "foreground": p := metav1.DeletePropagationForeground; do.PropagationPolicy = &p
            case "background": p := metav1.DeletePropagationBackground; do.PropagationPolicy = &p
            case "orphan": p := metav1.DeletePropagationOrphan; do.PropagationPolicy = &p
            }
        }
        if v, ok := req.Params["gracePeriodSeconds"]; ok {
            switch t := v.(type) {
            case float64: x := int64(t); do.GracePeriodSeconds = &x
            case int: x := int64(t); do.GracePeriodSeconds = &x
            case int64: x := t; do.GracePeriodSeconds = &x
            }
        }
        if req.DryRun || validateOnly { do.DryRun = []string{"All"} }
        if err := ri.Delete(ctx, ref.Name, do); err != nil {
            return dto.ItemResult{Ref: ref, Status: "error", HTTPStatus: statusFromError(err), Message: err.Error()}
        }
        if validateOnly { return dto.ItemResult{Ref: ref, Status: "ok", HTTPStatus: http.StatusOK, Message: "Delete validated"} }
    ...
    if strings.ToLower(req.Action) == "scale" {
        ...
        case float64: replicas = int64(v)
        case int: replicas = int64(v)
        case int32: replicas = int64(v)
        case int64: replicas = v
        default:
    ...
    if strings.ToLower(req.Action) == "export-yaml" {
        ...
        if err != nil { return dto.ItemResult{Ref: ref, Status: "error", HTTPStatus: statusFromError(err), Message: err.Error()} }
        item := dto.ItemResult{Ref: ref, Status: "ok", HTTPStatus: http.StatusOK, Message: "Exported"}
        if !validateOnly {
            if yb, err := yaml.Marshal(u.Object); err == nil { item.YAML = string(yb) }
        } else {
...
func (s *Server) evictPod(ctx context.Context, clients *k8s.ImpersonatedClients, ref dto.ObjectRef, dryRun bool) error {
    ...
    if dryRun { ev.DeleteOptions = &metav1.DeleteOptions{DryRun: []string{"All"}} }
...
func mapGenericToCoordinator(action, kind string) (string, string, string, bool) {
    ...
    case "restart":
        switch k { case "pod","pods": return "restart-pods", "pods", "delete", true
        case "deployment","deployments": return "restart-deployments", "deployments", "update", true
        case "daemonset","daemonsets": return "restart-daemonsets", "daemonsets", "update", true
        case "statefulset","statefulsets": return "restart-statefulsets", "statefulsets", "update", true }
        return "", "", "", false
    case "scale":
        switch k { case "deployment","deployments": return "scale-deployments", "deployments", "update", true
        case "statefulset","statefulsets": return "scale-statefulsets", "statefulsets", "update", true }
        return "", "", "", false
    case "suspend":
        if k == "cronjob" || k == "cronjobs" { return "suspend-cronjobs", "cronjobs", "update", true }
        return "", "", "", false
    case "resume":
        if k == "cronjob" || k == "cronjobs" { return "resume-cronjobs", "cronjobs", "update", true }
        return "", "", "", false
    case "cordon":
        if k == "node" || k == "nodes" { return "cordon-nodes", "nodes", "patch", true }
        return "", "", "", false
    case "uncordon":
        if k == "node" || k == "nodes" { return "uncordon-nodes", "nodes", "patch", true }
        return "", "", "", false
    case "drain":
        if k == "node" || k == "nodes" { return "drain-nodes", "nodes", "patch", true }
        return "", "", "", false
    case "export-yaml":
        return "export-yaml", plural, "get", true
    case "evict":
        if k == "pod" || k == "pods" { return "evict-pods", "pods", "delete", true }
        return "", "", "", false
    default:
        return "", "", "", false
    }
...
func toPluralResource(kindLower string) string {
    ...
    default:
        ...
        if strings.HasSuffix(kindLower, "s") { return kindLower }
        return kindLower + "s"
    }
}
